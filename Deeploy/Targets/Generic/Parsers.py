...
class MulParserBase(NodeParser):
...
class MulParser(MulParserBase):

    def parseNode(self, node: gs.Node) -> (bool):
        return super().parseNode(node) and node.inputs[0].shape == node.inputs[1].shape


class MulScalarParser(MulParserBase):
