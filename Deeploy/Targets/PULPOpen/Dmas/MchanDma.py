...
class MchanBarrier(Future):

    _initTemplate = NodeTemplate("uint32_t ${name} = mchan_get_channel_id();")
...
class MchanDma(AsyncDma):
    ...
    _waitingStrategy = BarrierWaitingStrategy(MchanBarrier, "channel_id")
    ...
    def transferOpRepr(self, externalBuffer: VariableBuffer, localBuffer: VariableBuffer, shape: Tuple[int, ...],
        ...
        assert mchanTransferSize <= 2**17, f"The Dma transfer size for mchan should be representable with 17 bits, current number of bits required is {math.ceil(math.log2(mchanTransferSize))}"
