...
class Conv2DTileConstraint(TileConstraint):
    ...
    def addGeometricalConstraint(tilerModel: TilerModel, parseDict: Dict, ctxt: NetworkContext) -> TilerModel:
        ...
        if 'bias' in parseDict:
            biasBufferName = parseDict["bias"]

            tilerModel.addTensorDimToModel(ctxt, biasBufferName)

            biasBuffer = ctxt.lookup(biasBufferName)
            assert isinstance(biasBuffer, ConstantBuffer)

            if len(biasBuffer.shape) == 4:
                biasBatchVar = tilerModel.getTensorDimVar(tensorName = biasBufferName, dimIdx = 0)
                biasChannelVar = tilerModel.getTensorDimVar(tensorName = biasBufferName, dimIdx = 1)
                biasHeightVar = tilerModel.getTensorDimVar(tensorName = biasBufferName, dimIdx = 2)
                biasWidthVar = tilerModel.getTensorDimVar(tensorName = biasBufferName, dimIdx = 3)

                tilerModel.addConstraint(biasBatchVar == 1)
                tilerModel.addConstraint(biasChannelVar == weightOutChannelVar)
                tilerModel.addConstraint(biasHeightVar == 1)
                tilerModel.addConstraint(biasWidthVar == 1)
            elif len(biasBuffer.shape) == 1:
                biasChannelVar = tilerModel.getTensorDimVar(tensorName = biasBufferName, dimIdx = 0)
                tilerModel.addConstraint(biasChannelVar == weightOutChannelVar)
            else:
                raise RuntimeError(f"Unsupported bias shape {biasBuffer.shape}")
