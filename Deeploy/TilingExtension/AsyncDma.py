...
class BarrierWaitingStrategy(AsyncDmaWaitingStrategy):
    ...
    def __init__(self, FutureCls: Type[Future], barrierName: str) -> None:
        ...
        self.barrierFuture = FutureCls(barrierName)

    def getFuture(self, tensorName: str, direction: DmaDirection) -> Future:
        _ = tensorName, direction
        return self.barrierFuture


class DirectionBarrierWaitingStrategy(AsyncDmaWaitingStrategy):

    def __init__(self, FutureCls: Type[Future], barrierName: str) -> None:
        super().__init__(FutureCls)
        self.inputBarrierFuture = FutureCls(f"{barrierName}_in")
        self.outputBarrierFuture = FutureCls(f"{barrierName}_out")

    def getFuture(self, tensorName: str, direction: DmaDirection) -> Future:
        _ = tensorName
        if direction == "ExternalToLocal":
            return self.inputBarrierFuture
        else:
            return self.outputBarrierFuture
...
class AsyncDma:
    ...
    def checkTransfer(self, ctxt: NetworkContext, externalBuffer: VariableBuffer, localBuffer: VariableBuffer,
        ...
        assert transferRank in self._transferTemplates, f"Unsupported transfer rank {transferRank}. Supported ranks are {self.supportedTransferRanks()}"
    ...
    def transferOpRepr(self, externalBuffer: VariableBuffer, localBuffer: VariableBuffer, shape: Tuple[int, ...],
                       ...
                       asyncToken: Future) -> OperatorRepresentation:
        return {"loc": localBuffer.name, "ext": externalBuffer.name, "async_token": asyncToken.name}
    ...
    def transfer(self, ctxt: NetworkContext, externalBuffer: VariableBuffer, localBuffer: VariableBuffer,
        ...
        opRepr = self.transferOpRepr(externalBuffer, localBuffer, shape, strideExt, strideLoc, direction, asyncToken)
...
class BlockingDmaFromAsyncDmaAdapter(AsyncDma):
    ...
    def transfer(self, ctxt: NetworkContext, externalBuffer: VariableBuffer, localBuffer: VariableBuffer,
                 ...
                 direction: DmaDirection, asyncToken: Future) -> List[CodeSnippet]:
        callStack = []
        callStack.append(asyncToken.init())
        callStack.extend(
            self.dma.transfer(ctxt, externalBuffer, localBuffer, shape, strideExt, strideLoc, direction, asyncToken))
        callStack.append(asyncToken.wait())
        callStack.append(asyncToken.deinit())
        return callStack
...
class MultidimDmaSnippetGenerator:
    ...
    def transfer(self, ctxt: NetworkContext, externalBuffer: VariableBuffer, localBuffer: VariableBuffer,
                 shape: Tuple[int, ...], strideExt: Tuple[int, ...], strideLoc: Tuple[int, ...],
                 direction: DmaDirection, asyncToken: Future, strideExtPad: int) -> List[CodeSnippet]:
        transferRank = len(shape)
        ...
        if kernelRank < transferRank:
            ...
            callStack.extend(
                self.dma.transfer(ctxt, externalBufferOffseted, localBufferOffseted, shape[-kernelRank:],
                                  strideExt[-kernelRank:], strideLoc[-kernelRank:], direction, asyncToken))
            callStack.append(CodeSnippet(self.NestedForLoopCloseTemplate(nestedLoopDepth), {}))
            return callStack
        elif kernelRank == transferRank:
            return self.dma.transfer(ctxt, externalBuffer, localBuffer, shape, strideExt, strideLoc, direction,
                                     asyncToken)
        else:
            return self.dma.transfer(ctxt, externalBuffer, localBuffer, padShape(shape, kernelRank),
                                     ...
                                     padStride(strideLoc, kernelRank, math.prod(shape)), direction, asyncToken)
